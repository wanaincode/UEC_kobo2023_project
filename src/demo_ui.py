...
os.makedirs(DATA_SAMPLE_FEATURES, exist_ok=True)  # safe to exist even if empty
...
def nms(bboxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float) -> torch.Tensor:
    """Simple NMS using IoU threshold."""
...
def ground_dino_predict(model, img_path, text_prompt, box_threshold=0.35, text_threshold=0.25, topK=10):
    """Run GroundingDINO to produce candidate boxes."""
...
def sam_predict(predictor, img_path, boxes):
    """Run SAM to get masks from boxes."""
...
def load_model_and_predict():
    """Load GroundingDINO, SAM, and CLIP extractor."""
...
def extract_saved_obj_features(model, predictor, extractor):
    """Extract features from registered object images under data/saved_obj_img."""
    ...
    if not osp.exists(output_path):
        ...
        # remove all files to avoid mixing old results
...
def recognize_pipeline(model, predictor, extractor, obj_features, recognize_img_path, box_threshold=0.35, text_threshold=0.25, idx=0):
    """Full recognition pipeline: detect -> mask -> embed -> cosine similarity -> annotate."""
    ...
    if os.path.exists(recognize_img_path):
        ...
        for obj_idx, (boxi, maski) in enumerate(zip(boxes, masks)):
            ...
            # cosine similarity against registered features
...
while True:
    ...
    # object image root path (under data/)
    input_path = DATA_SAVED_OBJ_IMG
    ...
    # show preview images (left strip)
    width = 150
    ...
    # Enter to start capturing the object
    ...
    # Recognize (button / mouse)
    if recognize_flag and process_flag[0] == "Waiting":
        process_flag[0] = "Recognizing"
        process_flag_color[0] = (0, 0, 255)

        # prefer packaged sample features if present; fallback to extracted features
        ...
        # save current test image under data/
